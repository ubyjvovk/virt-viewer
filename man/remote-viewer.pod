...
Hotkey key-names are composed of modifiers (shift, ctrl, alt, cmd or
meta) and a non-modifier key.  On macOS, C<cmd> and C<meta> both map to the
Command key (B<Option> is C<alt>): the Command key itself is consumed
by the host when pressed as part of a hotkey; the I<Send key> menu can
still synthesize Command-based combinations in the guest.
...
It must be composed of modifiers (shift, ctrl, alt, cmd or
meta) and a non-modifier key. For example, "shift+f11".  On macOS,
C<cmd> and C<meta> both map to the Command key (B<Option> is C<alt>).
